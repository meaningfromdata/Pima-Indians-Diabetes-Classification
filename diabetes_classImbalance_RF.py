...
### import sklearn functions for Random Forest Classifier and train/test split
...
### split into train/test (stratify by y to ensure split by Outcome)
...
y_classProbs = rf.predict_proba(X_test)




### Look at feature importance in classifier performance

importances = rf.feature_importances_
std = np.std([rf.feature_importances_ for feature in rf.estimators_],
             axis=0)
...
### show validation curves for different number of estimators used to fit KNN
from sklearn.learning_curve import validation_curve
depth = np.arange(1, 20)
train_score, val_score = validation_curve(RandomForestClassifier(), X, y, 'max_depth', depth, cv=10)
